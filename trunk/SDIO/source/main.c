...
SD_Error SD_Test(void)
{
	u8 * pt;
	int i;
	SD_Error Status=SD_OK;
	printf("Test:\n\rThis test reads 32X2 bytes from SD Card. Test if it is right.");
	if (Status==SD_OK)
	{
...
int main(void)
{
	...
	SD_GetPrepared();
	SD_Test();
	printf("\n\rFAT16 file system:the card have to be formatted by SDFormatter.\n\r");
	FAT_Init();
	FAT_Info();
	firstSec=DirStartSec();
	List_AllDir_Long(firstSec);
    while(1)
    {	
