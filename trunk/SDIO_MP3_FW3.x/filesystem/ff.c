...
)
{
	...
	if (dj->sect) {
		...
		fno->fdate = LD_WORD(dir+DIR_WrtDate);		/* Last Date */
		fno->ftime = LD_WORD(dir+DIR_WrtTime);		/* Last Time */
        fno->fcreatedate = LD_WORD(dir+DIR_CrtDate);		/* Create Date */ //ÐÂÌîÈë by godofsparta20100309
		fno->fcreatetime = LD_WORD(dir+DIR_CrtTime);		/* Create Time */
	}
	*p = 0;
